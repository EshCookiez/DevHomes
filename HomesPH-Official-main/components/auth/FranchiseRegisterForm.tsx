...
export default function FranchiseRegisterForm() {
  ...
  useEffect(() => {
    if (affiliateCode) {
      trackRecruitmentClick(affiliateCode, 'franchise', source, campaign).catch(console.error)
    }
      ...
      <div className="bg-white rounded-xl border-t-[3px] border-[#0c1f4a] shadow-[0_4px_6px_rgba(12,31,74,0.04),0_10px_40px_rgba(12,31,74,0.10),0_30px_60px_rgba(12,31,74,0.06)] ring-1 ring-[#0c1f4a]/[0.05] px-8 sm:px-10 py-9 sm:py-10">
        ...
        <div className="flex items-center gap-3 mb-6">
          <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center shrink-0">
            <Landmark size={20} className="text-[#0c1f4a]" />
          </div>
          <div>
            <h1 className="text-xl font-black text-[#0c1f4a] tracking-tight leading-tight">Franchise Registration</h1>
            <p className="text-xs text-gray-500 mt-0.5">Start your own real estate agency branch</p>
          </div>
        </div>
        ...
        {affiliateCode && (
          <div className="mb-6 px-4 py-2.5 rounded-lg bg-emerald-50 border border-emerald-100 flex items-center gap-2">
            ...
            <p className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider">
              Recruited by: <span className="font-black underline">{affiliateCode}</span>
            </p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
          ...
          <div className="pt-1.5 space-y-3">
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#0c1f4a] to-[#163880] text-white text-[15px] font-bold tracking-wide hover:from-[#0f2860] hover:to-[#1a44a0] active:scale-[0.99] hover:scale-[1.005] transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed disabled:scale-100 shadow-[0_4px_20px_rgba(12,31,74,0.28)] hover:shadow-[0_6px_30_rgba(12,31,74,0.4)]"
            >
              {loading ? (
