...
import { Eye, EyeOff, Mail, Lock, User, Phone, ShieldCheck, Award } from 'lucide-react'
...
const schema = z.object({
  ...
  prc_number: z.string().min(4, 'PRC license number is required.'),
    ...
    <div className="w-full max-w-[540px]">
      <div className="bg-white rounded-xl border-t-[3px] border-[#0c1f4a] shadow-[0_4px_6px_rgba(12,31,74,0.04),0_10px_40px_rgba(12,31,74,0.10),0_30px_60px_rgba(12,31,74,0.06)] ring-1 ring-[#0c1f4a]/[0.05] px-8 sm:px-10 py-9 sm:py-10">
        ...
        <div className="flex items-center gap-3 mb-6">
          <div className="w-10 h-10 rounded-xl bg-[#0c1f4a]/[0.06] flex items-center justify-center shrink-0">
            <Award size={20} className="text-[#0c1f4a]" />
          </div>
          <div>
            <h1 className="text-xl font-black text-[#0c1f4a] tracking-tight leading-tight">Broker Registration</h1>
            <p className="text-xs text-gray-500 mt-0.5">Licensed real estate broker</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
          ...
          {/* PRC License Number */}
          <div>
            <label className="block text-[12px] font-bold text-gray-500 mb-1 tracking-wide uppercase">PRC License Number</label>
            <div className="relative">
              <span className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400"><Award size={14} /></span>
              <input
                {...register('prc_number')}
                placeholder="e.g. 0012345"
                className={`w-full pl-9 pr-4 py-3 rounded-xl border text-[14px] text-gray-900 placeholder:text-gray-300 bg-gray-50/70 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#0c1f4a]/20 focus:border-[#0c1f4a] hover:border-gray-300 transition-all ${errors.prc_number ? 'border-rose-400 bg-rose-50/60' : 'border-gray-200'}`}
              />
            </div>
            {errors.prc_number && <p className="mt-1 text-xs text-rose-600">{errors.prc_number.message}</p>}
          </div>
