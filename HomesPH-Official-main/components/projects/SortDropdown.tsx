...
export default function SortDropdown() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen(v => !v)}
        className="flex items-center justify-between px-[12px] gap-[6px] border border-[#D3D3D3] rounded-[10px] bg-white text-[18px] font-outfit font-light text-[#002143] hover:bg-gray-50 transition-colors cursor-pointer shrink-0"
        style={{ width: '149.79px', height: '45.24px' }}
      >
        <div className="flex items-center gap-[6px]">
          <LayoutList size={22} className="text-[#002143]" />
          <span className="truncate max-w-[80px]">{currentLabel}</span>
        </div>
        <ChevronDown size={22} className={`text-[#002143] transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
